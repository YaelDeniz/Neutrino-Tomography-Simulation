...
public: 
    ...
    double SetRegion(); // {Emin, Emax, Etamin, Etamax, dAz}

    double SetDetParameters[3]; //{Mass, alphaE, alphaZ} + T?

    double SetTimeOfExp;

    double SetBins[2]; //{i energy bins , j angular bins}

    double SetEventType; // 0 Cascade-like, 1 Track Like 

    double SetEarthModel;
...
TH2D*  TrueEvents::GetTrueEvents(std::string model, int flvf, double E_GeV[], double Eta[], double dAz ,int Ebins, int Tbins, double NnT)
{  


    //std::string filename = "SimulationResults/NtrueAlt"+std::to_string(flvf)+"_"+std::to_string(Ebins)+std::to_string(Tbins)+".csv";

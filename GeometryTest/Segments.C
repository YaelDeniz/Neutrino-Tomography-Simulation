...
void Segments()
{
   ...
   std::vector< std::vector<double> > PremMatrix = SortPREMdata("prem_default.txt"); // Sort PREM model into a readable matrix
   ...
   double lbs = lb;
      ...
      if (layerb > lb )
      {
         ...
         { 
            ...
            srmin= lbs;
            srmax= layerb + excess;
            std::cout << "top of lower segment: " << srmin << " " << srmax << " "<< " " << lslabL << " " << srmax - srmin  << std::endl;
            r.clear();
            break;
         }
